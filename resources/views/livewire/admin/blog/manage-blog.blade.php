<div class="p-4">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-zuppie-700">Blog Posts ({{ $blogs->total() }})</h2>
    ...
    @if ($blogs->isEmpty())
        <p class="text-zuppie-pink-600 text-center py-8">No blog posts found.</p>
    @else
            ...
            <table class="min-w-full bg-white rounded-lg">
                ...
                <tbody class="divide-y divide-zuppie-pink-100">
                    @foreach ($blogs as $blog)
                        <tr class="hover:bg-zuppie-pink-50 transition" wire:key="blog-{{ $blog->id }}">
                            <td class="py-4 px-6 whitespace-nowrap">
                                @if ($blog->image)
                                    <img src="{{ $blog->image }}" alt="Blog image"
                                        class="h-12 w-12 object-cover rounded-lg border-2 border-zuppie-300 shadow">
                                @else
                            ...
                            <td class="py-4 px-6 whitespace-nowrap text-zuppie-pink-700">{{ $blog->slug }}</td>
        ...
        <div class="fixed inset-0 backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6 border-2 border-zuppie-pink-300">
                <div class="text-center">
                    <h3 class="text-lg font-bold text-zuppie-pink-700 mb-4">Are you sure you want to delete this Blog Post?
                    </h3>
