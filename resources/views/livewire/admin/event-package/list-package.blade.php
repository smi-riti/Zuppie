    ...
    <div class="">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('admin.event-packages.create') }}"
                class="px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition flex items-center shadow-lg">
        ...
        @if (session()->has('message'))
            <div
                class="mb-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-700 rounded-lg flex items-center shadow-sm">
                    ...
                    @foreach ($packages as $package)
                        <tr class="hover:bg-purple-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-purple-700">
                                {{ $package->category->name ?? 'None' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-purple-700">
                                ₹{{ number_format($package->price, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-purple-700">
                                ₹{{ number_format($package->discounted_price, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-purple-700">
                                {{ $package->formatted_duration }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if (count($package->images) > 0)
                                    <div class="flex space-x-2">
                                        <div class="relative">
                                            <img src="{{ $package->images->first()->image_url }}" alt="Package Image"
                                                class="h-14 w-14 object-cover rounded-lg shadow-sm border border-purple-200">
                                        </div>
                                        @if (count($package->images) > 1)
                                            <div class="relative">
                                                <img src="{{ $package->images[1]->image_url }}" alt="Package Image"
                                                    class="h-14 w-14 object-cover rounded-lg shadow-sm border border-purple-200">
                                ...
                                <div class="flex items-center space-x-4">
                                    <div>
                                        <label class="flex items-center cursor-pointer"
                                            wire:click.prevent="toggleActive({{ $package->id }})"
                                            wire:loading.class="opacity-50"
                                            wire:target="toggleActive({{ $package->id }})">
                                            <div
                                                class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                                                <div
                                                    class="block w-10 h-6 rounded-full shadow-inner {{ $package->is_active ? 'bg-purple-400' : 'bg-purple-200' }}">
                                                </div>
                                                <div
                                                    class="absolute inset-y-0 left-0 w-6 h-6 bg-white rounded-full shadow transform transition-transform duration-200 ease-in-out {{ $package->is_active ? 'translate-x-full' : '' }}">
                                                </div>
                                            </div>
                                            <span
                                                class="text-xs {{ $package->is_active ? 'text-purple-700 font-medium' : 'text-purple-500' }}">
                                    ...
                                    <div>
                                        <label class="flex items-center cursor-pointer"
                                            wire:click.prevent="toggleSpecial({{ $package->id }})"
                                            wire:loading.class="opacity-50"
                                            wire:target="toggleSpecial({{ $package->id }})">
                                            <div
                                                class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                                                <div
                                                    class="block w-10 h-6 rounded-full shadow-inner {{ $package->is_special ? 'bg-pink-400' : 'bg-pink-200' }}">
                                                </div>
                                                <div
                                                    class="absolute inset-y-0 left-0 w-6 h-6 bg-white rounded-full shadow transform transition-transform duration-200 ease-in-out {{ $package->is_special ? 'translate-x-full' : '' }}">
                                                </div>
                                            </div>
                                            <span
                                                class="text-xs {{ $package->is_special ? 'text-pink-700 font-medium' : 'text-pink-500' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <button wire:click="openViewModal({{ $package->id }})"
                                        class="px-3 py-1.5 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition shadow-sm flex items-center">
                                    ...
                                    <a href="{{ route('admin.event-packages.edit', $package->id) }}"
                                        class="px-3 py-1.5 bg-pink-100 text-pink-700 rounded-lg hover:bg-pink-200 transition shadow-sm flex items-center">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <button wire:click="openDeleteModal({{ $package->id }})"
                                        class="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition shadow-sm flex items-center">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div
                ...
                <div class="p-4 mb-5 bg-yellow-50 text-yellow-800 rounded-lg border border-yellow-200">
                    <p class="flex items-center">
                        ...
                        Are you sure you want to delete this package? This action will only soft delete the package and
