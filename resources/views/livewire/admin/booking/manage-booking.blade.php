<div class="min-h-screen bg-gradient-to-br from-zuppie-50 to-zuppie-pink-50 p-4 sm:p-6">
    <div class="">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-zuppie-800">Manage Bookings</h1>
                <p class="text-zuppie-600">View and manage all event bookings</p>
            </div>
            <button wire:click="openCreateModal"
                class="flex items-center px-4 py-2 bg-gradient-to-r from-zuppie-600 to-zuppie-pink-500 text-white rounded-lg shadow-md hover:from-zuppie-700 hover:to-zuppie-pink-600 transition-all">
        ...
        <!-- Bookings Table -->
        <div class="bg-white rounded-xl shadow-lg border border-zuppie-200">
            <div class="p-3 border-b border-zuppie-200 bg-zuppie-50">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-zuppie-600">
                        ...
                        @if (!empty($search) || !empty($statusFilter) || !empty($userFilter) || !empty($packageFilter))
                            <span class="ml-1">
                                ...
                                <button
                                    ...
                                    class="ml-2 text-zuppie-600 hover:text-zuppie-800 underline text-xs focus:outline-none">
                                    Clear filters
                                </button>
                            </span>
                        @endif
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-zuppie-200">
                    <thead class="bg-zuppie-50">
                        <tr>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-zuppie-700 uppercase tracking-wider">
                                Customer</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-zuppie-700 uppercase tracking-wider">
                                Event Details</th>
                    ...
                    <tbody class="bg-white divide-y divide-zuppie-200">
                        @forelse ($bookings as $booking)
                            <tr class="hover:bg-zuppie-50">
                                <!-- Customer Column -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            class="flex-shrink-0 h-10 w-10 rounded-full bg-gradient-to-r from-zuppie-400 to-zuppie-pink-400 flex items-center justify-center text-white font-bold">
                                            {{ strtoupper(substr($booking->booking_name, 0, 1)) }}
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-zuppie-900">{{ $booking->booking_name }}
                                            </div>
                                            <div class="text-sm text-zuppie-600">{{ $booking->booking_email }}</div>
                                            <div class="text-xs text-zuppie-500">{{ $booking->booking_phone_no }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-zuppie-900">
                                        {{ $booking->eventPackage->category->name ?? 'N/A' }} -
                                        {{ $booking->eventPackage->name }}</div>
                                    <div class="text-xs text-zuppie-500 mt-1">{{ $booking->guest_count }} guests</div>
                                    @if ($booking->special_requests)
                                        <div class="text-xs text-zuppie-pink-600 mt-1 italic">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-sm text-zuppie-600">{{ $booking->event_date->format('h:i A') }}
                                    </div>
                                </td>

                                <!-- Location/Pincode Column -->
                                <td class="px-6 py-4">
                                    <div class="text-sm text-zuppie-900">{{ Str::limit($booking->location, 20) }}
                                    </div>
                                    <div class="text-xs text-zuppie-500">{{ $booking->pin_code }}</div>
                                </td>
                                            ...
                                            @foreach ($statusOptions as $status)
                                                @if ($status !== $booking->status)
                                                    @if (
                                                        ...
                                                        <button
                                                            @click="if(confirm('Are you sure you want to change status to {{ $status }}? This action requires admin approval.')) { $wire.updateStatus({{ $booking->id }}, '{{ $status }}'); showStatusChangeModal = false; }"
                                                            class="block w-full text-left px-4 py-2 text-xs hover:bg-gray-100">
                                                            {{ ucfirst($status) }}
                                                        </button>
                                                    @else
                                                        <button
                                                            @click="$wire.updateStatus({{ $booking->id }}, '{{ $status }}'); showStatusChangeModal = false;"
                                                            class="block w-full text-left px-4 py-2 text-xs hover:bg-gray-100">
                                                            {{ ucfirst($status) }}
                                                        </button>
                                                    @endif
                                ...
                                <!-- Amount Column -->
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-zuppie-900">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('admin.booking.view', $booking->id) }}">
                                            <button
                                                class="p-2 bg-zuppie-100 text-zuppie-600 rounded-lg hover:bg-zuppie-200 transition shadow-sm">
                                        ...
                                        <button wire:click="confirmDelete({{ $booking->id }})"
                                            class="p-2 bg-zuppie-pink-100 text-zuppie-pink-600 rounded-lg hover:bg-zuppie-pink-200 transition shadow-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 text-center text-sm text-zuppie-600">
                                    No bookings found
                                </td>
                            </tr>
                        @endforelse
            ...
            <!-- Pagination -->
            <div class="px-6 py-4 border-t border-zuppie-200">
                {{ $bookings->links() }}
            </div>
        </div>
    </div>
    ...
    @if ($showUpdateModal)
        <livewire:admin.booking.update-booking :bookingId="$bookingIdToUpdate" :key="'update-booking-' . $bookingIdToUpdate" />
    @endif
