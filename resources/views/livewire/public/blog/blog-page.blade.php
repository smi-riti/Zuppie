<div class="min-h-screen bg-gray-50">
    ...
    <div class="relative bg-gradient-to-br from-pink-500 via-purple-600 to-purple-700 text-white py-16 sm:py-20 lg:py-24 overflow-hidden">
        <!-- Background Image -->
        ...
        <div class="absolute top-32 right-20 w-32 h-32 bg-pink-400/20 rounded-full blur-2xl animate-float-slow"></div>
            ...
            <div class="overflow-x-auto scrollbar-hide">
                <div class="flex space-x-3 pb-2" style="width: max-content;">
                    ...
                    <button wire:click="selectCategory('')"
                        class="flex-shrink-0 px-4 py-2 rounded-full border-2 transition-all duration-200 {{ !$selectedCategory ? 'bg-purple-500 border-purple-500 text-white' : 'bg-white border-gray-200 text-gray-700 hover:border-purple-300' }}">
                    ...
                    @foreach($categories as $category)
                        <button wire:click="selectCategory({{ $category->id }})"
                            class="flex-shrink-0 px-4 py-2 rounded-full border-2 transition-all duration-200 {{ $selectedCategory == $category->id ? 'bg-purple-500 border-purple-500 text-white' : 'bg-white border-gray-200 text-gray-700 hover:border-purple-300' }}">
                            <div class="flex items-center space-x-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="hidden lg:block lg:col-span-1">
                <div class="sticky top-8">
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                        ...
                        <!-- All Categories Button -->
                        <button wire:click="selectCategory('')"
                            class="w-full p-3 mb-3 text-left rounded-xl transition-all duration-200 {{ !$selectedCategory ? 'bg-purple-100 text-purple-700 ring-1 ring-purple-500' : 'bg-gray-50 hover:bg-gray-100 text-gray-700' }}">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    <div class="w-8 h-8 rounded-lg {{ !$selectedCategory ? 'bg-purple-500' : 'bg-gray-300' }} flex items-center justify-center">
                        ...
                        @foreach($categories as $category)
                            <button wire:click="selectCategory({{ $category->id }})"
                                class="w-full p-3 mb-2 text-left rounded-xl transition-all duration-200 {{ $selectedCategory == $category->id ? 'bg-purple-100 text-purple-700 ring-1 ring-purple-500' : 'bg-gray-50 hover:bg-gray-100 text-gray-700' }}">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-8 h-8 rounded-lg {{ $selectedCategory == $category->id ? 'bg-purple-500' : 'bg-gray-300' }} flex items-center justify-center">
                                ...
                                <div class="space-y-2">
                                    @if($search)
                                        <div class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-purple-100 text-purple-800">
                                            ...
                                            "{{ $search }}"
                                            <button wire:click="$set('search', '')" class="ml-2 text-purple-600 hover:text-purple-800">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                </svg>
                                            </button>
                        ...
                        @foreach($blogs as $blog)
                            <article class="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 border border-gray-100">
                                ...
                                <div class="relative h-44 sm:h-48 overflow-hidden">
                                    ...
                                    @else
                                        <div class="w-full h-full bg-gradient-to-br from-purple-100 via-purple-200 to-purple-300 flex items-center justify-center">
                                            <svg class="w-10 h-10 sm:w-12 sm:h-12 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($blog->category)
                                        <div class="absolute top-3 left-3">
                                            <span class="inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium bg-white/90 backdrop-blur-sm text-purple-700 shadow-sm">
                                                {{ $blog->category->name }}
                                            </span>
                                        </div>
                                    @endif
                                ...
                                <div class="p-4 sm:p-6">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <a href="/blog/{{ $blog->slug }}" 
                                           class="inline-flex items-center px-3 sm:px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-medium rounded-lg text-xs sm:text-sm transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105">
                                            Read More
                                        ...
                                        <!-- Share Button -->
                                        <button class="p-2 text-gray-400 hover:text-purple-500 hover:bg-purple-50 rounded-lg transition-all duration-300"
                ...
                @else
                    ...
                    <div class="text-center py-16">
                        <div class="max-w-md mx-auto">
                            <div class="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-6 bg-gradient-to-br from-purple-100 to-purple-200 rounded-full flex items-center justify-center">
                            ...
                            @if($search || $selectedCategory)
                                ...
                                <button wire:click="clearFilters" 
                                    class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-medium rounded-xl transition-all duration-300 shadow-md hover:shadow-lg">
