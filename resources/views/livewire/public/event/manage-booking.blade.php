<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50">
    ...
    <!-- Success Message -->
    @if($showSuccess)
        <div class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-30 backdrop-blur-sm z-50 flex items-center justify-center p-4" id="success-overlay">
            <div class="bg-white/95 backdrop-blur-lg rounded-3xl p-8 max-w-md w-full text-center shadow-2xl border border-white/20" id="success-modal">
                <div class="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
                    <i class="fas fa-check text-green-600 text-4xl"></i>
                </div>
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Booking Confirmed!</h2>
                <p class="text-gray-600 mb-6">Your event booking has been successfully confirmed. We'll contact you soon to finalize the details.</p>
                <div class="w-full bg-gray-200 rounded-full h-2 mb-4">
                    <div class="bg-gradient-to-r from-purple-600 to-pink-600 h-2 rounded-full" id="progress-bar" style="width: 0%"></div>
                </div>
                <button wire:click="dismissSuccess" class="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 rounded-xl font-semibold">
                    Continue
                </button>
            </div>
        </div>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="bg-gradient-to-r from-purple-600 to-pink-600 p-8 text-white">
                            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                                <div>
                                    ...
                                    <p class="text-purple-100">Booking ID: #{{ str_pad($booking->id, 6, '0', STR_PAD_LEFT) }}</p>
                                </div>
                        ...
                        <div class="p-8">
                            <div class="grid lg:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                                            ...
                                            <span class="px-3 py-1 rounded-full text-sm font-medium {{ $this->bookingStatusBadge }}">
                                        ...
                                        @if($booking->event_end_date && $booking->event_end_date != $booking->event_date)
                                            <div class="flex items-center justify-between py-3 border-b border-gray-100">
                                                ...
                                                <span class="font-semibold">{{ $booking->event_end_date->format('M d, Y') }}</span>
                                            </div>
                                        @endif
                                        ...
                                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                                            ...
                                            <span class="font-bold text-lg text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">
                                        ...
                                        @if($booking->payments->count() > 0)
                                            <div class="flex items-center justify-between py-3">
                                                ...
                                                <span class="px-3 py-1 rounded-full text-sm font-medium {{ $this->paymentStatusBadge }}">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            <label class="block text-gray-600 text-sm font-medium mb-2">Customer Information</label>
                                        ...
                                        @if($booking->special_requests)
                                            <div>
                                                <label class="block text-gray-600 text-sm font-medium mb-2">Special Requests</label>
                                        ...
                                        @if($booking->eventPackage->images->count() > 0)
                                            <div>
                                                <label class="block text-gray-600 text-sm font-medium mb-2">Package Preview</label>
                                ...
                                <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
                                    @if($userBooking->eventPackage->images->count() > 0)
                                        <img src="{{ $userBooking->eventPackage->images->first()->image_url }}" 
                                             alt="{{ $userBooking->eventPackage->name }}" 
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <span class="px-3 py-1 rounded-full text-xs font-medium {{ $userBooking->status === 'confirmed' ? 'bg-green-100 text-green-800' : ($userBooking->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                                                {{ ucfirst($userBooking->status) }}
                                            </span>
                                            <span class="font-bold text-purple-600">₹{{ number_format($userBooking->total_price) }}</span>
                                        </div>
            ...
            @if(!$booking)
                <div class="text-center py-20">
                    ...
                    <p class="text-gray-600 mb-8">You don't have any bookings yet. Start by exploring our amazing event packages.</p>
