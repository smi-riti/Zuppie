<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50">
    ...
    <section class="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div class="relative z-10 w-full flex flex-col items-center px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto text-center">
            <h1 class="text-4xl sm:text-6xl md:text-8xl font-black text-white leading-tight mb-6 animate-fade-in">
                ...
                <span class="block text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-purple-400 to-pink-500">
                    Event Packages
                </span>
            </h1>
            <p class="text-lg sm:text-xl md:text-2xl text-gray-100 max-w-3xl mx-auto leading-relaxed mb-8 drop-shadow-md">
            ...
            <div class="w-full max-w-lg mx-auto mb-8">
                <div class="relative group">
                    <input type="text" wire:model.live.debounce.500ms="searchQuery" 
                           wire:keydown.enter="searchPackages"
                    ...
                    <button wire:click="searchPackages" class="absolute right-6 top-1/2 -translate-y-1/2 transition-colors duration-300 group-hover:text-pink-300">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
                @foreach($this->specialCategories as $category)
                    <div wire:click="openCategoryModal('{{ $category['slug'] }}')" 
                        ...
                        <div class="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 bg-gradient-to-br from-purple-100 to-pink-100 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                            <i class="{{ $category['icon'] }} text-lg sm:text-2xl text-purple-600 group-hover:text-pink-600"></i>
                        </div>
                        <h3 class="text-sm sm:text-base font-semibold text-gray-800 group-hover:text-purple-600 transition-colors">
                            {{ $category['name'] }}
                        </h3>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row justify-between items-center mb-10">
                ...
                <div class="flex items-center mt-4 md:mt-0 space-x-2">
                    <button id="popular-prev" class="p-2 bg-white border border-gray-300 rounded-full shadow-md hover:bg-gray-50 transition-all duration-300">
                        <i class="fas fa-chevron-left text-gray-600"></i>
                    </button>
                    <button id="popular-next" class="p-2 bg-white border border-gray-300 rounded-full shadow-md hover:bg-gray-50 transition-all duration-300">
            ...
            <div class="relative overflow-hidden">
                <div id="popular-carousel" class="flex transition-transform duration-500 ease-in-out space-x-6">
                    @foreach($this->popularPackages as $package)
                        <div class="flex-shrink-0 w-80 bg-white rounded-2xl shadow-xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl">
            ...
            <section class="py-16 {{ $loop->even ? 'bg-white' : 'bg-gray-50' }}">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex flex-col md:flex-row justify-between items-center mb-10">
                        ...
                        <div class="flex items-center mt-4 md:mt-0 space-x-2">
                            <button class="category-prev p-2 bg-white border border-gray-300 rounded-full shadow-md hover:bg-gray-50 transition-all duration-300" 
                                    data-category="{{ $category['slug'] }}">
                                <i class="fas fa-chevron-left text-gray-600"></i>
                            </button>
                            <button class="category-next p-2 bg-white border border-gray-300 rounded-full shadow-md hover:bg-gray-50 transition-all duration-300" 
                                    data-category="{{ $category['slug'] }}">
                                <i class="fas fa-chevron-right text-gray-600"></i>
                            </button>
                        </div>
                    ...
                    <div class="relative overflow-hidden">
                        <div id="category-{{ $category['slug'] }}-carousel" class="flex transition-transform duration-500 ease-in-out space-x-6 category-carousel">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            setupCarousel('popular-carousel', 'popular-prev', 'popular-next');
            ...
            document.querySelectorAll('.category-carousel').forEach(carousel => {
                ...
                setupCarousel(carousel.id, `[data-category="${categorySlug}"].category-prev`, `[data-category="${categorySlug}"].category-next`);
            });

            function setupCarousel(carouselId, prevSelector, nextSelector) {
                ...
                const maxTranslate = -(carousel.children.length - Math.floor(carousel.parentElement.clientWidth / itemWidth)) * itemWidth;

                prevBtn.addEventListener('click', () => {
                    currentTranslate = Math.min(currentTranslate + itemWidth * 2, 0);
                    carousel.style.transform = `translateX(${currentTranslate}px)`;
                });

                nextBtn.addEventListener('click', () => {
                    currentTranslate = Math.max(currentTranslate - itemWidth * 2, maxTranslate);
                    carousel.style.transform = `translateX(${currentTranslate}px)`;
                });
