<div>
    @if($showModal)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-md z-40 transition-all duration-300" wire:click="closeModal">
        </div>
    @endif
    ...
    <div class="@if(!$showModal) hidden @endif fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        <div class="relative mx-auto w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <!-- Modal Content -->
            <div
                class="bg-white rounded-2xl shadow-2xl border-0 overflow-hidden transform transition-all animate-in zoom-in-95 duration-300">
                <!-- Modal Header -->
                <div class="bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-5">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-purple-100 text-sm mt-1">Tell us about your vision and we'll make it happen
                            </p>
                        </div>
                        <button wire:click="closeModal"
                            class="text-white/80 hover:text-white transition-colors rounded-full p-2 hover:bg-white/20">
                ...
                @if (session('message'))
                    <div
                        class="mx-6 mt-4 px-4 py-3 rounded-xl bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 font-medium border border-green-200 flex items-center">
        ...
        skipField() {
            if (this.currentField === 'event_type') this.currentField = 'budget';
            else if (this.currentField === 'budget') this.currentField = 'contact_info';
        },
        ...
        init() {
            this.updateBudgetRange(this.formData.budget_index);
                    ...
                    <div x-show="currentField === 'event_type'">
                        <label class="block text-gray-800 font-2xl mb-4 text-lg">What type of event are you planning?
                            *</label>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            @foreach ($parentCategory as $category)
                                <div x-on:click="formData.event_type = '{{ $category->slug }}'; 
                                    $wire.set('formData.event_type', '{{ $category->slug }}'); 
                                    autoNext('budget')"
                    ...
                    <div x-show="currentField === 'budget'" class="space-y-4">
                        <label class="block text-gray-800 font-2xl mb-4 text-lg">What's your estimated budget range?
                            *</label>
                        <div class="px-2">
                            <input wire:model="formData.budget_index" type="range"
                                x-on:input="updateBudgetRange($event.target.value); autoNext('contact_info')" min="0"
                                max="5" step="1"
                    ...
                    <div x-show="currentField === 'contact_info'" class="space-y-6">
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                @error('formData.phone')<span
                                class="text-red-500 text-sm">{{ $message }}</span>@enderror
                            </div>

                            <!-- Message now inside Contact Info -->
                            <div class="md:col-span-2">
                        ...
                        <div class="flex justify-between items-center">
                            <button type="button" @click="currentField = 'budget'"
                                ...
                                Back</button>
                            <button type="submit"
                                class="w-fit bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 px-4 rounded-lg text-lg hover:shadow-lg hover:scale-[1.02] transition-all flex items-center justify-center space-x-2">
