<div
    ...
    <div class="max-w-md w-full space-y-8 bg-white rounded-2xl shadow-xl p-8">
        @if(!$emailSent)
            ...
            @if($error)
                <div class="text-sm text-pink-600 bg-pink-50 border border-pink-200 rounded-lg p-3 text-center">
                    {{ $error }}
                </div>
            @endif
            ...
            <form wire:submit.prevent="sendResetLink" class="space-y-6">
                <div>
                    ...
                    <input wire:model="email" id="email" type="email" autofocus class="mt-1 block w-full px-4 py-3 border border-purple-300 rounded-lg text-purple-900
        ...
        @else
            <div class="text-center">
                ...
                <a href="{{ route('login') }}"
                    class="mt-4 inline-block text-sm font-medium text-purple-600 hover:text-purple-800 transition duration-200">
                    Back to Login
                </a>
            </div>
        @endif
